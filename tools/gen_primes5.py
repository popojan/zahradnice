#!/usr/bin/env python3
"""Generate programs/primes5/index.cfg -- Fischer's real-time prime array.
...
def emit(out):
    ...
    # ---- header ----------------------------------------------------------
    add('#!Fischer 1965   primes: {score} {parallel}  {help}')
    ...
    add('# 2 rewrites per column whatever the height -- raise #threads to')
    add('# spend that parallelism (the screen comes out the same either way).')
    add('#')
    add('# glyphs:')
    for k in sorted(gl.of, key=lambda z: (z[0], z)):
        add('#   %-3s %s' % (gl.of[k], k))
    add('#')
    add('#help  space=pause  c=restart  q=quit')
    add('#timing T 3')
    add('#threads 1')
    add('#control Z pause')
    add('#control C clear')
